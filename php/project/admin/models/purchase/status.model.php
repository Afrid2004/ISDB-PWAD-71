...
class Status extends Model implements JsonSerializable{
	...
	public function save(){
		global $db,$tx;
		$db->query("insert into {$tx}statuss(name)values('$this->name')");
		return $db->insert_id;
	}
	public function update(){
		global $db,$tx;
		$db->query("update {$tx}statuss set name='$this->name' where id='$this->id'");
	}
	...
	public static function pagination($page=1,$perpage=10,$criteria=""){
		global $db,$tx;
		$top=($page-1)*$perpage;
		$result=$db->query("select id,name from {$tx}statuss $criteria limit $top,$perpage");
		$data=[];
		while($status=$result->fetch_object()){
			$data[]=$status;
		}
	...
	public static function find($id){
		global $db,$tx;
		$result =$db->query("select id,name from {$tx}statuss where id='$id'");
...
";
	...
	static function html_table($page = 1,$perpage = 10,$criteria="",$action=true){
		...
		while($status=$result->fetch_object()){
			...
			$html.="<tr><td>$status->id</td><td>$status->name</td> $action_buttons</tr>";
		}
		$html.="</table>";
