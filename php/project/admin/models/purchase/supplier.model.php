...
class Supplier extends Model implements JsonSerializable{
	...
	public function set($id,$name,$mobile,$email,$address){
		$this->id=$id;
		$this->name=$name;
		$this->mobile=$mobile;
		$this->email=$email;
	...
	public function save(){
		global $db,$tx;
		$db->query("insert into {$tx}suppliers(name,mobile,email,address)values('$this->name','$this->mobile','$this->email','$this->address')");
		return $db->insert_id;
	}
	public function update(){
		global $db,$tx;
		$db->query("update {$tx}suppliers set name='$this->name',mobile='$this->mobile',email='$this->email',address='$this->address' where id='$this->id'");
	}
	...
	public static function all(){
		global $db,$tx;
		$result=$db->query("select id,name,mobile,email,address from {$tx}suppliers");
		$data=[];
	...
	public static function pagination($page=1,$perpage=10,$criteria=""){
		global $db,$tx;
		$top=($page-1)*$perpage;
		$result=$db->query("select id,name,mobile,email,address from {$tx}suppliers $criteria limit $top,$perpage");
		$data=[];
		while($supplier=$result->fetch_object()){
			$data[]=$supplier;
		}
	...
	public static function find($id){
		global $db,$tx;
		$result =$db->query("select id,name,mobile,email,address from {$tx}suppliers where id='$id'");
...
";
	...
	static function html_table($page = 1,$perpage = 10,$criteria="",$action=true){
		...
		if($action){
			$html.="<tr><th>Id</th><th>Name</th><th>Mobile</th><th>Email</th><th>Address</th><th>Action</th></tr>";
		}else{
			$html.="<tr><th>Id</th><th>Name</th><th>Mobile</th><th>Email</th><th>Address</th></tr>";
		}
		while($supplier=$result->fetch_object()){
			$action_buttons = "";
			if($action){
			...
			$html.="<tr><td>$supplier->id</td><td>$supplier->name</td><td>$supplier->mobile</td><td>$supplier->email</td><td>$supplier->address</td> $action_buttons</tr>";
		}
		$html.="</table>";
