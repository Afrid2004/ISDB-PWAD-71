...
class Stock extends Model implements JsonSerializable{
	...
	public function set($id,$product_id,$qty,$transaction_type_id,$remark,$created_at,$warehouse_id,$lot_id){
		$this->id=$id;
	...
	public function save(){
		global $db,$tx;
		$db->query("insert into {$tx}stocks(product_id,qty,transaction_type_id,remark,created_at,warehouse_id,lot_id)values('$this->product_id','$this->qty','$this->transaction_type_id','$this->remark','$this->created_at','$this->warehouse_id','$this->lot_id')");
		return $db->insert_id;
	}
	public function update(){
		global $db,$tx;
		$db->query("update {$tx}stocks set product_id='$this->product_id',qty='$this->qty',transaction_type_id='$this->transaction_type_id',remark='$this->remark',created_at='$this->created_at',warehouse_id='$this->warehouse_id',lot_id='$this->lot_id' where id='$this->id'");
	}
	...
	public static function find($id){
		global $db,$tx;
		$result =$db->query("select id,product_id,qty,transaction_type_id,remark,created_at,warehouse_id,lot_id from {$tx}stocks where id='$id'");
...
";
	...
	static function html_table($page = 1,$perpage = 10,$criteria="",$action=true){
		...
		if($action){
			$html.="<tr><th>Id</th><th>Product Id</th><th>Qty</th><th>Transaction Type Id</th><th>Remark</th><th>Created At</th><th>Warehouse Id</th><th>Lot Id</th><th>Action</th></tr>";
		}else{
			$html.="<tr><th>Id</th><th>Product Id</th><th>Qty</th><th>Transaction Type Id</th><th>Remark</th><th>Created At</th><th>Warehouse Id</th><th>Lot Id</th></tr>";
		}
		while($stock=$result->fetch_object()){
			$action_buttons = "";
			if($action){
			...
			$html.="<tr><td>$stock->id</td><td>$stock->product_id</td><td>$stock->qty</td><td>$stock->transaction_type_id</td><td>$stock->remark</td><td>$stock->created_at</td><td>$stock->warehouse_id</td><td>$stock->lot_id</td> $action_buttons</tr>";
		}
		$html.="</table>";
