...
";
	...
	static function html_select($name ="customer_id")
	{
		global $db, $tx;
		$data = $db->query("select * from core_customers");
		$html = " <select name='$name' id='$name' class='form-select'>";
		while ($row = $data->fetch_object()) {
			$html .= "<option data-email='$row->email'  data-mobile='$row->mobile' data-address='$row->address' value='$row->id'>$row->name</option>";
		}
		return  $html .= "  </select>";
	}
	...
	static function html_table($page = 1, $perpage = 10, $criteria = "", $action = true)
	{
		...
		while ($customer = $result->fetch_object()) {
			...
			$html .= "<tr><td>$customer->id</td><td>$customer->name</td><td>$customer->mobile</td><td>$customer->email</td> $action_buttons</tr>";
		}
		$html .= "</table>";
