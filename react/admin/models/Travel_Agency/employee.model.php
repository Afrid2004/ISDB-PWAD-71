...
class Employee extends Model implements JsonSerializable{
	...
	public function save(){
		global $db,$tx;
		$db->query("insert into {$tx}employees(name,phone,designation)values('$this->name','$this->phone','$this->designation')");
		return $db->insert_id;
	}
	public function update(){
		global $db,$tx;
		$db->query("update {$tx}employees set name='$this->name',phone='$this->phone',designation='$this->designation' where id='$this->id'");
	}
	...
	public static function all(){
		global $db,$tx;
		$result=$db->query("select id,name,phone,designation from {$tx}employees");
		$data=[];
	...
	public static function pagination($page=1,$perpage=10,$criteria=""){
		global $db,$tx;
		$top=($page-1)*$perpage;
		$result=$db->query("select id,name,phone,designation from {$tx}employees $criteria limit $top,$perpage");
		$data=[];
		while($employee=$result->fetch_object()){
			$data[]=$employee;
		}
	...
	public static function find($id){
		global $db,$tx;
		$result =$db->query("select id,name,phone,designation from {$tx}employees where id='$id'");
...
";
	...
	static function html_table($page = 1,$perpage = 10,$criteria="",$action=true){
		...
		if($action){
			$html.="<tr><th>Id</th><th>Name</th><th>Phone</th><th>Designation</th><th>Action</th></tr>";
		}else{
			$html.="<tr><th>Id</th><th>Name</th><th>Phone</th><th>Designation</th></tr>";
		}
		while($employee=$result->fetch_object()){
			$action_buttons = "";
			if($action){
			...
			$html.="<tr><td>$employee->id</td><td>$employee->name</td><td>$employee->phone</td><td>$employee->designation</td> $action_buttons</tr>";
		}
		$html.="</table>";
