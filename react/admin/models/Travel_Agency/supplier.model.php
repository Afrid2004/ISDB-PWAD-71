<?php
class TMSSupplier extends Model implements JsonSerializable{
	public $id;
	public $company_name;
	public $phone;
	public $email;
	...
	public function set($id,$company_name,$phone,$email){
		$this->id=$id;
	...
	public function save(){
		global $db,$tx;
		$db->query("insert into {$tx}suppliers(company_name,phone,email)values('$this->company_name','$this->phone','$this->email')");
		return $db->insert_id;
	}
	public function update(){
		global $db,$tx;
		$db->query("update {$tx}suppliers set company_name='$this->company_name',phone='$this->phone',email='$this->email' where id='$this->id'");
	}
	...
	public static function all(){
		global $db,$tx;
		$result=$db->query("select id,company_name,phone,email from {$tx}suppliers");
		$data=[];
	...
	public static function pagination($page=1,$perpage=10,$criteria=""){
		global $db,$tx;
		$top=($page-1)*$perpage;
		$result=$db->query("select id,company_name,phone,email from {$tx}suppliers $criteria limit $top,$perpage");
		$data=[];
		while($supplier=$result->fetch_object()){
			$data[]=$supplier;
		}
	...
	public static function find($id){
		global $db,$tx;
		$result =$db->query("select id,company_name,phone,email from {$tx}suppliers where id='$id'");
...
";
	...
	static function html_table($page = 1,$perpage = 10,$criteria="",$action=true){
		...
		if($action){
			$html.="<tr><th>Id</th><th>Company Name</th><th>Phone</th><th>Email</th><th>Action</th></tr>";
		}else{
			$html.="<tr><th>Id</th><th>Company Name</th><th>Phone</th><th>Email</th></tr>";
		}
		while($supplier=$result->fetch_object()){
			$action_buttons = "";
			if($action){
			...
			$html.="<tr><td>$supplier->id</td><td>$supplier->company_name</td><td>$supplier->phone</td><td>$supplier->email</td> $action_buttons</tr>";
		}
		$html.="</table>";
	...
	static function html_row_details($id){
		...
		$html.="<tr><th>Company Name</th><td>$supplier->company_name</td></tr>";
